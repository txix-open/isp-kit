...
type ClusteredBootstrap struct {
	*Bootstrap
	ClusterCli   ClusterClient
	RemoteConfig *rc.Config
}

func NewClustered(moduleVersion string, remoteConfig any, endpoints []cluster.EndpointDescriptor) *ClusteredBootstrap {
	isDev := isOnDevMode()
